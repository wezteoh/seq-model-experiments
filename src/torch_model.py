"""
adapted from mamba2-minimal https://github.com/tommyip/mamba2-minimal
"""
...
class CustomMixerModel(nn.Module):
    ...
    def token_generate(
        ...
        if n_chunked > 0:
            h = self(prefix[:n_chunked].unsqueeze(0), None, inference=True).hidden_states
        else:
            h = [
        ...
        for i in range(n_chunked, prefix.shape[0]):
            h = self(prefix[i : i + 1].unsqueeze(0), h, inference=True).hidden_states
...
def segsum(x: Tensor, device: Device = None) -> Tensor:
    ...
    mask = torch.tril(torch.ones(T, T, dtype=torch.bool, device=device), diagonal=-1).to(device)
    x = x.masked_fill(~mask, 0)
    x_segsum = torch.cumsum(x, dim=-2)
    mask = torch.tril(torch.ones(T, T, dtype=torch.bool, device=device), diagonal=0).to(device)
