...
try:
    ...
    def cauchy_keops(v, z, w, backend="GPU"):
    ...
    cauchy_keops_gpu = partial(cauchy_keops, backend="GPU")
    cauchy_keops_cpu = partial(cauchy_keops, backend="CPU")
    log_vandermonde_keops_gpu = partial(log_vandermonde_keops, backend="GPU")
    log_vandermonde_keops_cpu = partial(log_vandermonde_keops, backend="CPU")
    log_vandermonde_transpose_keops_gpu = partial(log_vandermonde_transpose_keops, backend="GPU")
    log_vandermonde_transpose_keops_cpu = partial(log_vandermonde_transpose_keops, backend="CPU")
...
class SSMKernel(Kernel):
    ...
    def __init__(
        ...
        **init_args,
        ...
        super().__init__(
            d_model=d_model, channels=channels, l_max=l_max, lr=lr, wd=wd, verbose=verbose
        )
        self.N = d_state
...
class SSMKernelDiag(SSMKernel):
    ...
    def forward(self, L, state=None, rate=1.0):
        ...
        elif has_pykeops and self.backend == "cuda":
            log_vandermonde = log_vandermonde_keops_gpu
        elif has_pykeops and self.backend == "keops":
            log_vandermonde = log_vandermonde_keops_cpu
        else:
    ...
    def forward_state(self, u, state):
        ...
        # Dispatch which Vandermonde kernel to use
        if has_pykeops and self.backend == "cuda":
            log_vandermonde_transpose = log_vandermonde_transpose_keops_gpu
        elif has_pykeops and self.backend == "keops":
            log_vandermonde_transpose = log_vandermonde_transpose_keops_cpu
        else:
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        if (
            ...
            cauchy_mult = cauchy_keops_gpu
        elif has_pykeops and self.backend == "keops":
            cauchy_mult = cauchy_keops_cpu
        else:
    ...
    def _setup_step(self, mode="dense"):
        ...
        if mode == "linear":
            ...
            if self.verbose:
                print(
                    "Diagonalization error:", torch.dist(V @ torch.diag_embed(L) @ V_inv, self.dA)
                )
...
class S4Block(nn.Module):
    ...
    def __init__(
        ...
        **layer_args,  # Arguments into inner layer (e.g. FFTConv)
        ...
        self.layer = FFTConv(
            d_model, transposed=False, dropout=dropout, tie_dropout=tie_dropout, **layer_args
        )
    ...
    def forward(self, x, lengths=None, **kwargs):  # absorbs return_output and transformer src mask
        ...
        if lengths is not None:
            ...
            mask = torch.where(
                torch.arange(L, device=lengths.device)[:, None] < lengths[:, None, None], 1.0, 0.0
            )
            x = x * mask
